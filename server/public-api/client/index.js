...
module.exports = class ClientController {
  constructor(app) {
    ...
    app.post('/api/add-temporary-data', this.addTemporaryData);
    // app.post('/api/add-temporary-purchase-data', this.addTemporaryPurchaseData);


  }


  addTemporaryData(req, res) {
    let isSaleInsert = true;
    let isPurchaseInsert = true;
    let month = req.body.month;
    let Client = new db.TemporaryDashboard();

    db.TemporaryDashboard.findOne({ "GSTNo": req.body.gst }).then((record) => {
      if (_.size(record)) {
        if (req.body.category == 1) {
          if (record.saleFile) { // IF SALE FILE OBJECT FOUND THEN PUSH/UPDATE
            _.forEach(record.saleFile[month], function(value) {
              if ((value.Invoice_Number == req.body.data.Invoice_Number && value.Email_Address == req.body.data.Email_Address)) {
                isSaleInsert = false;
              }
            })
            if (isPurchaseInsert == true) {
              record.saleFile[month].push(req.body.data);
            }
          } else { //IF SALE FILE OBJECT NOT FOUND THEN INSERT
            let temp = [];
          ...
          if (isSaleInsert == true) {
            db.TemporaryDashboard.update({ "GSTNo": req.body.gst }, { $set: record }).then((response) => {
              res.send({ recordDetails: record });
            }).catch((error) => {
              res.send(error);
            })
          } else {
          ...
          if (record.purchaseFile) { // IF SALE PURCHASE OBJECT FOUND THEN PUSH/UPDATE
            _.forEach(record.purchaseFile[month], function(value) {
              if ((value.Invoice_Number == req.body.data.Invoice_Number && value.Email_Address == req.body.data.Email_Address)) {
                isPurchaseInsert = false;
              }
            })
            console.log("isPurchaseInsert ==>", isPurchaseInsert);
            if (isPurchaseInsert == true) {
              record.purchaseFile[month].push(req.body.data);
            }
          } else { //IF PURCHASE FILE OBJECT NOT FOUND THEN INSERT
            let temp = [];
            temp.push(req.body.data);
            let obj = {};
            obj[month + ''] = temp;
            record.purchaseFile = obj;
          }
      ...
      } else { //IF NOT RECORD FOUND THEN CREATE NEW RECORD AND INSERT PERTICULAR FILE
        Client.GSTNo = req.body.gst;
let temp = [];
temp.push(req.body.data);
let obj = {};
obj[month + ''] = temp;
if (req.body.category == 1) {
  ...
  Client.purchaseFile = obj;
}
Client.save().then((resp) => {
res.send({ recordDetails: resp });
}).catch((error) => {
res.send(error);
})
}
  ...
  // addTemporarySaleData(req, res) {
  //   console.log("addTemporarySaleData");
  //   let isInsert = true;
  //   let month = req.body.month;
  //   let Client = new db.TemporaryDashboard();
  //   db.TemporaryDashboard.findOne({ "GSTNo": req.body.gst }).then((record) => {
  //     if (_.size(record)) {
  //       if (_.size(record.saleFile)) {
  //         _.forEach(record.saleFile[month], function(value) {

  //           console.log("value.Email_Address-------------", value);

  //           console.log("req.body.data.Email_Address------------", req.body.data.Email_Address);
  //           if ((value.Invoice_Number == req.body.data.Invoice_Number && value.Email_Address == req.body.data.Email_Address)) {
  //             isInsert = false;
  //           }
  //         })
  ...
  // addTemporaryPurchaseData(req, res) {
  //   let isInsert = true;
  //   let month = req.body.month;
  //   let Client = new db.TemporaryDashboard();
  //   db.TemporaryDashboard.findOne({ "GSTNo": req.body.gst }).then((record) => {
  //     if (_.size(record)) {
  //       if (_.size(record.purchaseFile)) {
  //         _.forEach(record.purchaseFile[month], function(value) {
  //           console.log("req.body.data.Email_Address------------", req.body.data.Email_Address);
  //           if ((value.Invoice_Number == req.body.data.Invoice_Number && value.Email_Address == req.body.data.Email_Address)) {
  //             isInsert = false;
